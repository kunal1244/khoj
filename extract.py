...
def straighten(image):
	...
	i,ctrs,hier = cv2.findContours(dilated.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def extractLines(img):
	...
	for i, line in enumerate(lines):
		...
		print int(line[0])
		print int(line[1])
		print len(hpList[int(line[0]):int(line[1])])
		print hpList[int(line[0]):int(line[1])]
	
	for i, line in enumerate(lines):
		cv2.imshow("line "+str(i), img[int(line[0]):int(line[1]), : ])
	'''
	...
	for i, line in enumerate(lines):
		...
		anchor = int(line[0]) # 'anchor' will locate the horizontal indices where horizontal projection is > ANCHOR_POINT for uphill or < ANCHOR_POINT for downhill(ANCHOR_POINT is arbitrary yet suitable!)
		...
		for x in range(1, len(anchorPoints)-1, 2):
			...
			lineTop = lineBottom
		if(int(line[1])-lineTop < 20):
			continue
		fineLines.append([lineTop, int(line[1])])
